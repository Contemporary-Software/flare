...
void flare_datasafe_init() {
    ...
    memcpy(
        &datasafe->boot_loader[6], flare_build_id(),
...
void flare_datasafe_factory_data_set(
    const uint8_t* mac_0, const uint8_t* mac_1, const uint8_t* mac_2,
    const uint8_t* mac_3, const char* ass_serial, const char* part,
    const char* revision, const char* mod, const char* board_serial,
    const char* app_data, const char* boot_cmd) {
    flare_datasafe* datasafe = (flare_datasafe*)FLARE_DS_BASE;
    size_t o;
    ...
    memcpy(
        &datasafe->assembly_serial_number[0], ass_serial,
    ...
    memcpy(
        &datasafe->board_serial_number[0], board_serial,
        FLARE_DS_FACTORY_DATA_LEN - 1);
    memcpy(
        &datasafe->app_data[0], app_data,
        FLARE_DS_FACTORY_APP_DETAILS_SIZE - 1);
    memcpy(
        &datasafe->boot_cmd[0], boot_cmd,
        FLARE_DS_FACTORY_APP_DETAILS_SIZE - 1);
...
void flare_datasafe_factory_data_clear() {
    ...
    memset(
        &datasafe->assembly_serial_number[0], 0,
    ...
    memset(
        &datasafe->board_serial_number[0], 0,
