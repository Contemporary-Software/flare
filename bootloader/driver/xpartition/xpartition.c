...
void xpart_load_parts() {
    ...
    for (partition = 1; partition < partition_count; partition++) {
        ...
        status = read_boot_bin(
            img.parts[partition].part_hdr_off * sizeof(uint32_t), &part_hdr,
        ...
        if (XPART_DEBUG_MODE) {
            for (int i = 0; i < XPART_NAME_LEN; i++) {
                part_name[(i * 4) + 0] =
                    (img.parts[partition].name[i] >> 24) & 0xFF;
                part_name[(i * 4) + 1] =
                    (img.parts[partition].name[i] >> 16) & 0xFF;
                part_name[(i * 4) + 2] =
                    (img.parts[partition].name[i] >> 8) & 0xFF;
                part_name[(i * 4) + 3] =
                    (img.parts[partition].name[i] >> 0) & 0xFF;
            }
        ...
        for (uint32_t sec = 0; sec < part_hdr.sec_count; sec++) {
            ...
            status = read_boot_bin(
                sec_hdr.next_part_off * sizeof(uint32_t), &sec_hdr,
                sizeof(struct part_header));
            if (status != 0) {
