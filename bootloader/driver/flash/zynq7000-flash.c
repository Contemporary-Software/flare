...
void flash_writeUnlock(void) {
    ...
    board_reg_write(
        reg, ((~(1 << GPIO_FLASH_WD_PIN) & 0xffff) << 16) |
...
flash_error flash_Transfer(flash_transfer_buffer* transfer, bool initialised) {
    ...
    if (initialised == false) {
        ...
        qspi_FlushRx();
        qspi_reg_write(
            QSPI_REG_CONFIG, QSPI_CR_SSFORCE | QSPI_CR_MANSTRTEN |
     ...
     */
    qspi_reg_write(
        QSPI_REG_CONFIG, qspi_reg_read(QSPI_REG_CONFIG) & ~QSPI_CR_PCS);
    ...
    if (start) {
        qspi_reg_write(
            QSPI_REG_CONFIG, qspi_reg_read(QSPI_REG_CONFIG) | QSPI_CR_MANSTRT);
