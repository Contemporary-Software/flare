...
void flash_transfer_trace(
    const char* message, const flash_transfer_buffer* transfer) {
#if FLASH_TRACE_TRANSFER
    if (flash_trace) {
        size_t c;
        printf(
            " %s: length=%ld in=%ld out=%ld padding=%ld\n", message,
...
static flash_error flash_TransferBuffer_Fill(
    flash_transfer_buffer* transfer, const uint8_t data, size_t length) {
...
static flash_error flash_TransferBuffer_CopyOut(
    flash_transfer_buffer* transfer, uint8_t* data, const size_t length) {
...
static flash_error flash_TransferBuffer_CopyIn(
    flash_transfer_buffer* transfer, const uint8_t* data, const size_t length) {
#ifdef FLARE_ZYNQMP
...
static void flash_TransferBuffer_SetAddr(
    flash_transfer_buffer* transfer, uint32_t address) {
#if FLASH_4BYTE_ADDRESSING
...
static void
flash_TransferBuffer_SetDir(flash_transfer_buffer* transfer, int trans_dir) {
    transfer->trans_dir = trans_dir;
}

static void flash_TransferBuffer_SetCommandLen(
    flash_transfer_buffer* transfer, int comm_len) {
    transfer->command_len = comm_len;
}

static void flash_TransferBuffer_SetCommMethod(
    flash_transfer_buffer* transfer, int comm_method) {
...
static flash_error flash_readCFI(void) {
    ...
    if (!cfi_data) {
        ...
        flash_TransferBuffer_Clear(flash_buf);
        flash_TransferBuffer_SetLength(
            flash_buf, FLASH_COMMAND_SIZE + cfi_length);
        flash_TransferBuffer_Set8(flash_buf, FLASH_READ_ID);
    ...
    printf(
        "   Flash CFI: ID: %02x %02x %02x Flash size: 0x%llx\n",
...
flash_error flash_open(const char** label) {
    ...
    *label = NULL;
    ...
    case 0x20:
        ...
        case 0x21:
            *label = "2x N25Q00A (128MiB) in parallel (256MiB total)";
            flash_size =
                0x8000000UL * 2UL; /* 1 Gib on each flash in parallel */
        ...
        case 0x22:
            *label = "2x mt25qu02g (256MiB) in parallel (512MiB total)";
            flash_size =
                0x10000000UL * 2UL; /* 2Gib on each flash in parallel */
    ...
    if (*label == NULL) {
        printf(
            "error: flash: unknown device: 0x%02x 0x%02x 0x%02x\n",
...
flash_error flash_read(uint32_t address, void* buffer, size_t length) {
    ...
    while (length) {
        ...
        flash_TransferBuffer_Clear(flash_buf);
        flash_TransferBuffer_SetLength(
            flash_buf, FLASH_COMMAND_SIZE + FLASH_ADDRESS_SIZE +
        ...
        if (fe != FLASH_NO_ERROR)
            return fe;
        flash_TransferBuffer_SetDir(flash_buf, FLASH_RX_TRANS);
        flash_TransferBuffer_SetCommandLen(
            flash_buf,
            FLASH_COMMAND_SIZE + FLASH_ADDRESS_SIZE + flash_read_dummies);
        ...
        flash_transfer_buffer_skip(
            flash_buf, FLASH_ADDRESS_SIZE + flash_read_dummies);
...
flash_writeSector(uint32_t address, const void* buffer, size_t length) {
    ...
    while (length) {
        ...
        if (write_block) {
            ...
            flash_TransferBuffer_Clear(flash_buf);
            flash_TransferBuffer_SetLength(
                flash_buf, FLASH_COMMAND_SIZE + FLASH_ADDRESS_SIZE + size);
            flash_TransferBuffer_Set8(flash_buf, FLASH_WRITE_CMD);
            flash_TransferBuffer_SetAddr(flash_buf, address);
            flash_TransferBuffer_SetDir(flash_buf, FLASH_TX_TRANS);
            flash_TransferBuffer_SetCommandLen(
                flash_buf, FLASH_COMMAND_SIZE + FLASH_ADDRESS_SIZE);
            flash_TransferBuffer_SetCommMethod(
                flash_buf, FLASH_COMM_METHOD_ALL);
...
flash_error flash_read_id(
    uint32_t* manufactureCode, uint32_t* memIfaceType, uint32_t* density) {
    uint8_t value = 0;
    flash_error fe;
