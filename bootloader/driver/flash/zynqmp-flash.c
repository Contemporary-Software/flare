...
static uint32_t command_word_generator(
    uint8_t imm_data, uint8_t data_xfer, uint8_t exp, uint8_t mode,
    uint8_t cs_lower, uint8_t cs_upper, uint8_t bus_sel, uint8_t tx, uint8_t rx,
...
static uint32_t command_wrapper(
    uint8_t parallel, uint8_t imm_data, uint8_t exp, uint8_t txrx,
    uint8_t stripe) {
    uint8_t bus;
    uint8_t cs_upper;
    uint8_t cs_lower;
...
flash_error flash_Transfer(flash_transfer_buffer* transfer, bool initialised) {
    ...
    if (initialised == false || true) {
        ...
        qspi_reg_write(
            GQSPI_LPBK_DLY_ADJ_OFST, GQSPI_LPBK_DLY_ADJ_USE_LPBK_MASK);
        initialised = true;
    }
    ...
    controller_command = command_wrapper(
        communication_method, header_len, IMMD_SIZE, FLASH_TX_TRANS, NOSTRIPE);
    ...
    while (length != 0) {
        if (length <= 0xFF) {
            ...
            controller_command = command_wrapper(
                communication_method, transfer_length, IMMD_SIZE, trans_dir,
                STRIPE);
        } else {
            /*
            ...
            controller_command = command_wrapper(
                communication_method, transfer_length, EXP_SIZE, trans_dir,
                STRIPE);
        }
    ...
    if (trans_dir == FLASH_TX_TRANS) {
        while (tx_length) {
            ...
            qspi_reg_write(
                GQSPI_CONFIG_OFST, qspi_reg_read(GQSPI_CONFIG_OFST) |
