...
static sdhci_error cmd_transfer(
    ...
    sdhci_reg_write(
        ctlr, SDHCI_INT_STATUS, SDHCI_INT_NORMAL_MASK | SDHCI_INT_ERROR_MASK);
    ...
    if (res != NULL) {
        *res = sdhci_reg_read(ctlr, SDHCI_RESPONSE0);
        SDHCI_TRACE_DEBUG(
            "sdhci (%d): cmd_transfer: SDHCI_NO_ERROR: 0x%08x\n", ctlr, *res);
    } else {
...
static sdhci_error read_data_transfer(int ctlr, char* buffer) {
    SDHCI_TRACE_DEBUG(
        "sdhci (%d): read_data_transfer(buffer = %p)\n", ctlr, buffer);
    uint32_t status;
    uint64_t end_time;
    uint64_t curr_time;
    board_timer_get(&curr_time);
...
static sdhci_error initialise_sd(int ctlr) {
     ...
     */
    err = cmd_transfer(
        ctlr, CMD9, (sdhcis[ctlr].card_rca << 16), 0,
...
static sdhci_error initialise_emmc(int ctlr) {
    ...
    /* Enable status bits */
    sdhci_reg_write(
        ctlr, SDHCI_INT_ENABLE, SDHCI_INT_NORMAL_MASK | SDHCI_INT_ERROR_MASK);
    ...
    while (!(res & SDHCI_OCR_READY)) {
        arg =
            SDHCI_OCR_SECTOR_MODE | SDHCI_OCR_2_7_3_6_V | SDHCI_OCR_1_7_1_95_V;
    ...
    if (csd_struct == 0) {
        ...
        printf(
            "CSD version not supported: %d: Card not attached\n", csd_struct);
        return SDHCI_CARD_NOT_SUPPORTED;
    }
...
sdhci_error
sdhci_read(int ctlr, uint32_t sector, uint32_t count, char* buffer) {
    SDHCI_DEBUG(
        "sdhci (%d): read(sector = 0x%08x, count = %d, buffer = %p)\n", ctlr,
        sector, count, buffer);
    sdhci_error err;
    uint32_t res;
