...
void factory_config_load(void) {
     ...
     */
    fe = flash_read(
        flash_device_size() - flash_device_sector_erase_size(), buffer,
        sizeof(flare_factory_data));
    ...
    if (data->crc32 == crc) {
        flare_datasafe_factory_data_set(
            ...
            &data->revision[0], &data->modstrike[0],
            &data->board_serial_number[0], &data->app_data[0],
            &data->boot_cmd[0]);
    } else {
        flare_datasafe_factory_data_clear();
        printf(
            "error: invalid factory data checksum, %08x != %08x\n", data->crc32,
            crc);
    }
}
