...
static inline void cache_l2_enable(void) {
    if ((board_reg_read(L2CC_BASE + L2CC_CNTRL) & 0x01) == 0) {
        ...
        board_reg_write(
            L2CC_BASE + L2CC_TAG_RAM_CNTRL, L2CC_TAG_RAM_DEFAULT_MASK);
        board_reg_write(
            L2CC_BASE + L2CC_DATA_RAM_CNTRL, L2CC_DATA_RAM_DEFAULT_MASK);
        board_reg_write(
            L2CC_BASE + L2CC_IAR, board_reg_read(L2CC_BASE + L2CC_ISR));
        board_reg_write(
            L2CC_BASE + L2CC_CNTRL, board_reg_read(L2CC_BASE + L2CC_CNTRL) | 1);
        cache_l2_sync();
        cache_dsb();
    }
}
