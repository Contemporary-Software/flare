...
static void boot_plan_clear(flare_boot_plan* bp) {
...
static void jtag_boot(flare_boot_plan* bp) {
    bool pressed;
    printf(
        "     JTAG boot: booting from %s in %1d   (^c for %s)"
    ...
    if ((pressed && JTAG_BOOT_DEFAULT != FLARE_DS_BOOTMODE_QSPI) ||
        ...
        flare_datasafe_set_bootmode(
            FLARE_DS_BOOTMODE_JTAG | FLARE_DS_BOOTMODE_QSPI);
        qspi_boot(bp);
    } else {
        flare_datasafe_set_bootmode(
            FLARE_DS_BOOTMODE_JTAG | FLARE_DS_BOOTMODE_SD_CARD);
        sdhci_boot(bp);
    }
}
